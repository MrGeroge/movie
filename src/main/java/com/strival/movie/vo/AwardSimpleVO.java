package com.strival.movie.vo;

import java.util.LinkedList;
import java.util.List;

/**
 * Created by xinghai on 2015/12/28.
 */
public class AwardSimpleVO {
    private long id;
    private String category;
    private List<String> prizeList=new LinkedList<String>();
    private int number;
    private String description;
    private long periodNum;

    public long getId() {
        return id;
    }

    public void setId(long id) {
        this.id = id;
    }

    public String getCategory() {
        return category;
    }

    public void setCategory(String category) {
        this.category = category;
    }

    public List<String> getPrizeList() {
        return prizeList;
    }

    public void setPrizeList(List<String> prizeList) {
        this.prizeList = prizeList;
    }

    public int getNumber() {
        return number;
    }

    public void setNumber(int number) {
        this.number = number;
    }

    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    public long getPeriodNum() {
        return periodNum;
    }

    public void setPeriodNum(long periodNum) {
        this.periodNum = periodNum;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;

        AwardSimpleVO that = (AwardSimpleVO) o;

        if (id != that.id) return false;
        if (number != that.number) return false;
        if (periodNum != that.periodNum) return false;
        if (!category.equals(that.category)) return false;
        if (!prizeList.equals(that.prizeList)) return false;
        return description.equals(that.description);

    }

    @Override
    public int hashCode() {
        int result = (int) (id ^ (id >>> 32));
        result = 31 * result + category.hashCode();
        result = 31 * result + prizeList.hashCode();
        result = 31 * result + number;
        result = 31 * result + description.hashCode();
        result = 31 * result + (int) (periodNum ^ (periodNum >>> 32));
        return result;
    }

    @Override
    public String toString() {
        return "AwardSimpleVO{" +
                "id=" + id +
                ", category='" + category + '\'' +
                ", prizeList=" + prizeList +
                ", number=" + number +
                ", description='" + description + '\'' +
                ", periodNum=" + periodNum +
                '}';
    }
}
